...
- Cloudinaryサムネイル変換を追加（`cloudinary_thumb` Jinja2フィルタ）
  - カード画像URLに `c_fill,w_520,h_320,f_auto,q_auto` を自動付与
  - 原寸画像でなくサムネイルを配信するため転送量が大幅削減
- `loading="lazy"` をカード画像に追加（スクロール外の画像を遅延読み込み）
